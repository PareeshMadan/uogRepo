...
{
	register int i;
	//allocate the control data for the threads
	struct TokenRingData *control = malloc(sizeof(struct TokenRingData));
	 ...
	 */
	for (i = 0; i < N_NODES; i++) {
		INITIALIZE_SEM(control, EMPTY(i), 1);  // Initially, buffers are empty
    	INITIALIZE_SEM(control, FILLED(i), 0); // No data in buffers initially
    	INITIALIZE_SEM(control, TO_SEND(i), 1); // Allow adding packets
	}
...
int
runSimulation(control, numberOfPackets, thrd)
	...
	pthread_t * thrd;
{
	 ...
	 */
	for (i = 0; i < N_NODES; i++){
		//allocate the arguments for the threads
		struct ArgumentsToken * arguments = malloc(sizeof(struct ArgumentsToken));
		arguments->num = i;
		arguments->control = control;
		//create the threads
		pthread_create(&thrd[i],NULL,token_node,arguments);
	}
